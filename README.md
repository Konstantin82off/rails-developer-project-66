...
На бесплатном тарифе Render нет возможности запуска отдельного фонового воркера. Для решения этой задачи проект использует **Solid Queue в режиме async**:

- Джобы выполняются асинхронно в том же процессе, что и веб-сервер
- Не требует дополнительных воркеров или платного тарифа
- Время ответа сервера сократилось с 5-10 секунд до <250 мс

При необходимости масштабирования можно переключиться на режим с отдельным воркером (mode: inline → mode: async + отдельный процесс).
